/**
 * Discogs 글로벌 인기 LP 20개 가져오기 (중복 제외)
 */
...
const SEARCH_QUERIES = [
  'type=release&format=LP&sort=want&sort_order=desc&style=Rock',
  'type=release&format=LP&sort=want&sort_order=desc&style=Jazz',
  'type=release&format=LP&sort=want&sort_order=desc&style=Pop',
  'type=release&format=LP&sort=want&sort_order=desc&style=Soul',
  'type=release&format=LP&sort=want&sort_order=desc&style=Electronic',
];
...
async function run() {
  ...
  for (const query of SEARCH_QUERIES) {
    ...
    for (let page = 1; page <= 5 && inserted < TARGET; page++) {
      ...
      if (!res.ok) { console.warn(`  ⚠️ ${res.status}`); break; }
